...
export default function Deposit({ chains }: DepositProps) {
  ...
  const { allowance, approve } = useERC20Allowance({
    ...
    spender: (chains[0].contracts!.optimismPortalProxy! as ChainContract)
      .address,
  });

  async function fetchBalance() {
    const balance = Number(
      await getBalance("l1", address!, token, chains[0].rpcUrls.default.http[0])
    );
    setBalance(balance);
  }
  ...
  async function estimateGas() {
    const contract = new web3.eth.Contract(
      optimismPortalProxyABI,
      (chains[0].contracts!.optimismPortalProxy as ChainContract).address
    );
